...
if (isset($_GET['action']) && $_GET['action'] == 'remove' && isset($_GET['id'])) {
    ...
    try {
        // Only persist changes for logged-in users; guests remain session-only
        ...
        if ($user_id) {
            ...
            $find = $conn->prepare('SELECT id FROM carts WHERE user_id = :user_id AND status <> "completed" ORDER BY created_at DESC LIMIT 1');
...
if (isset($_GET['action']) && $_GET['action'] == 'checkout') {
    ...
    // Only logged-in users persist carts to DB and can proceed to checkout directly
    if (!$user) {
        // Guest user: do not persist cart to DB, ask them to login to continue
        $_SESSION['info'] = 'Vui lòng đăng nhập để hoàn tất thanh toán.';
        header('Location: login.php');
        exit;
    }

    // Persist session cart into carts/cart_items for logged-in user
    try{
        require_once __DIR__ . '/../config/database.php';
        $sessionId = session_id();
        $user_id = $user['id'];
        ...
        if ($found && !empty($found['id'])){
            ...
            $up = $conn->prepare('UPDATE carts SET total_amount = :total_amount, updated_at = :updated_at WHERE id = :id');
            $up->execute([':total_amount'=>$totalMoney, ':updated_at'=>date('Y-m-d H:i:s'), ':id'=>$cartId]);
